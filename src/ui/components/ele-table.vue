        ...
        <el-table ref="ref_table" :span-method="spanMethod" :data="tableData" :style="{'width':tableWidth}" :height="maxHeight" :key="componentKey" border :row-class-name="setRowClassName">
            ...
            <template v-for="(col,idx) in columnConfig" v-if="col.show?col.show(ctx):true">
                ...
                <!-- <el-table-column v-else-if="col.displayHeader" :key="idx" :label="col.label" :width="col.width" :align="col.align?col.align:'center'">
                    ...
                    <template slot-scope="scope">
                        ...
                        <template v-else-if="col.type=='operate'">
                            ...
                            <el-button :type='item.btnType?item.btnType:"text"' v-for="(item,index) in col.operates" :key="index" class="tableButton" style="margin:0 5px;" :style="getStyle(item)" v-html="item.display(scope,ctx)" @click="item.click?item.click(scope,ctx):''" :disabled="item.disabled?item.disabled(scope,ctx):false" size="mini"></el-button>
                        </template>
                        <template v-else>
                ...
                <el-table-column v-else :key="idx" :label="col.label" :width="col.width" :align="col.align?col.align:'center'">
                    <template slot-scope="scope">
                        <template v-if="col.displayHeader" slot="header">
                            <div>
                                <span v-html="col.displayHeader(scope)"></span>
                            </div>
                        </template>
                        <template v-else-if="col.type=='index'">{{scope.$index+1}}</template>
