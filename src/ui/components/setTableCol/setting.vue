<template>
	<div>
		<el-dialog :title="item.name" :close-on-click-modal="false" center
				   ...
				   :before-close="close">
			<div style="color:#2e67f6">建议配置最多{{col||7}}列，否则可能会引起样式错乱</div>
			<div class="drag-box">
				<div class="drag-top">
 					<draggable
						:list="list1"
...
<script>
    ...
    export default {
        ...
        methods: {
			...
			reset(){
				this.list1=this.setting[this.item.key]
                //重置 hideFields
                this.gethideFields()
            },
            close() {
			...
			gethideFields(){
                ...
                map(allField, (item, key) => {
                    if (  !item.reference && !item.unConfigurable) {
                        let obj={key:key,width:item.width,label:item.text,type:'simple'}
                        if(item.slot){
                            obj={key:key,width:item.width,label:item.text,type:'slot'}
                        }
                        if(topListobj[key]){
