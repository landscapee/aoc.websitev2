<template>
    <div class="poolMonitorWithRunway">
        ...
        <el-dropdown class="positionDropdown" trigger="click" :hide-on-click="false">
            ...
            <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-for="(opt) in pageListObj" :key="opt.key">
                    <el-checkbox @change="showChange" v-model="poolMonitorShow[opt.key]"
                                 :label="opt.name"></el-checkbox>
                </el-dropdown-item>
        ...
        <div class="toprunway">
            ...
            <div class="runway" v-for="(opt) in runway" :key="opt.runway">
                    ...
                    <template v-for=" (opt,key) in noRunWay">
                        <template v-for=" (item,index) in opt">
                            ...
                            <div :class="'flightBox flightBox'+key+' ' " :key="opt+index">
                                <div class="norunwaytime "><span
                                    v-if="key==2">{{ item.movement }}</span><span>{{ tranTime(item) }}</span></div>
                                <div :class="'  flightNo ' + item.movement"><span class="fo">{{ item.flightNo }}</span>
                                </div>
                            </div>
                        </template>
                    </template>
    ...
    computed: {
        ...
        getLeft() {
            return (opt) => {
                ...
                let time2 = Math.ceil((time1 - time) / 60 / 1000) * this.timeItemWidth
                // console.log(time1, time1 / 60);
                return {left: time2 / 100 + 'rem', zIndex: opt.zIndex,}
            }
        },
        zhezhaoWidth() {
            let num = 15 - this.moveNum;
            if (num > this.runwayTime) {
                num = this.runwayTime
            } else if (num < 0) {
                num = 0
            }
            return {width: num * this.timeItemWidth / 100 + 'rem'}
        },
        notSeting() {
            return (opt) => {
