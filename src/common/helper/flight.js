...
/**
 * 航班延误判定, 分进港和离港
 * 对于已经落地或者起飞的, 能够准确的判断出是否延误
 * 但是对于未落地的, 只能判断当前时间以前(进港10分钟前, 离港30分钟前)的航班是否延误
 * 对于今后的航班是否会延误未知
 * 进港延误判定条件: 实际落地比计划到港时间 -10 分钟大的航班判定为进港延误
 * 离港延误判定条件: 实际起飞比计划离港时间 +30 分钟大的航班判定为离港延误
 * @param  {[type]} flight [description]
 * @return {[type]}        [description]
 */
export const calcDelay = (flight) => {
	let now = getTime();
	let movement = get(flight, 'movement');
	if (movement === 'A') {
		...
		let stdLimit = get(flight, 'std') + 30 * 60 * 1000;
		if (atd) {
			return atd > stdLimit;
		} else if (now > stdLimit) {
			return true;
		} else {
			return null;
		}
...
/**
 * 计算航班的标准保障时间, 超过该值则记为放行延误
 * 始发航班: 计划离港 + 30分钟推出后滑行
 * 未延误航班: 计划离港 + 30分钟推出后滑行
 * 到港延误: 计划停场时间 + 10分钟入位滑行 + 30分钟推出后滑行
 * 可以这样理解, sta 是到港落地时间, std 是离港关舱门时间
 * 现在新增2个字段来替换以前的标准保障时间
 * 以前的标准保障时间的理解是错误的, 实际上是起飞时间, 我还需要一个保障的截止时间
 *  takeOffDeadline, offBlockDeadline
 *  分别是对应到目标起飞时间和目标下轮挡时间
 *  其分别就是要不要加上30分钟的滑行时间
 */
...
/**
 * 计算放行延误, 只针对离港航班
 * 如果已经离港, 必然要么是始发航班, 或者已到港航班, 我们是能够取到该航班的最大保障时间
 * 但是对于不是始发, 并且还为落地的航班, 我们并不知道该航班
 *
 * @param  {[type]} flight [description]
 * @return {[type]}        [description]
 */
export const calcTakeOffDelay = (flight) => {
	let now = getTime();
	let movement = get(flight, 'movement');

	let atd = get(flight, 'atd');
	let limit = get(flight, 'maintainTime');
	if (atd) {
		return atd > limit;
...
export const calcTakeOffDelayTime = (flight) => {
	let now = getTime();
	let movement = get(flight, 'movement');
	...
	let limit = get(flight, 'maintainTime');
	if (atd && atd > limit) {
		return parseInt((atd - limit) / 60 / 1000);
	} else if (limit && !atd && now > limit) {
		return parseInt((now - limit) / 60 / 1000);
	} else {
		return null;
	}
};
 ...
 */
export const calcPercent = (flights) => {
	let reportFlights = filter(flights, (f) => {
		return get(f, 'includeInPercentReport');
	});
	let rate = reduce(
		reportFlights,
		(result, flight) => {
			let flightId = get(flight, 'flightId');
			let departureDelay = calcDelay(flight);
			if (departureDelay !== null) {
				result.departureTotal++;
				get(flight, 'originated') && result.originatedTotal++;
				get(flight, 'originated') && get(flight, 'morningBusyHourFlight') && result.morningBusyHourTotal++;
				if (departureDelay) {
					result.departureDelay++;
					result.departureDelayDetail.push(flightId);
					if (get(flight, 'originated')) {
						result.originatedDelay++;
						result.originatedDelayDetail.push(flightId);

						if (get(flight, 'morningBusyHourFlight')) {
							result.morningBusyHourDelay++;
							result.morningBusyHourDelayDetail.push(flightId);
						}
					}
				}
			}
			let takeOffDelay = calcTakeOffDelay(flight);
			if (departureDelay !== null && takeOffDelay !== null) {
				result.takeOffTotal++;
				if (takeOffDelay) {
					result.takeOffDelay++;
					result.takeOffDelayDetail.push(flightId);
				}
			}
			return result;
		},
		{
			...
			takeOffDelay: 0,
			takeOffDelayDetail: [],
			morningBusyHourTotal: 0,
			morningBusyHourDelay: 0,
			morningBusyHourDelayDetail: [],
		},
	);
	return Promise.resolve(rate);
};

export const calcDepartureDelayDetail = (flights) => {
	let result = {};
	each(flights, (flight) => {
		let departureDelay = calcDelay(flight);
		if (departureDelay !== null) {
			result[get(flight, 'flightId')] = departureDelay;
		}
	});
	return Promise.resolve(result);
};

export const calcTakeOffDelayDetail = (flights) => {
	let result = {};
	each(flights, (flight) => {
		let departureDelay = calcTakeOffDelay(flight);
		if (departureDelay !== null) {
			result[get(flight, 'flightId')] = departureDelay;
		}
	});
...
/**
 * 处理对于非IMIS中航班属性,
 * 如空管的 COBT CTOT AllowBoarding 等属性
 */
export const addExtendDisplayFiled = (flights) => {
	let result = map(flights, (data) => {
		each(['TOBT', 'COBT', 'allowBoarding', 'COBTByATC', 'CTOTByATC'], (key) => {
			let orgValue = get(data, key);
			if (orgValue) {
				set(data, `display${upperFirst(key)}`, moment(orgValue, 'x').format('HH:mm'));
				set(data, `display${upperFirst(key)}WithDate`, moment(orgValue, 'x').format('HH:mm(DD)'));
			} else {
				set(data, `display${upperFirst(key)}`, null);
				set(data, `display${upperFirst(key)}WithDate`, null);
			}
		});
		each(['T', 'Q'], (key) => {
			let orgValue = get(data, key);
			orgValue && set(data, key, toLower(orgValue) === 'true');
		});
		return data;
	});
	return Promise.resolve(result);
