...
const useVirtual = ({
	...
	// 获取某个子项的高度
	const getItemHeight = (index) => (isNumber(itemHeight) ? itemHeight : itemHeight(index));

	// 子项距离滚动区域顶部的距离、滚动区域的真实高度
	const [offsetTopCaches, wrapperHeight] = useMemo(() => [new Map(), getTotalHeight()], [total, itemHeight]);
